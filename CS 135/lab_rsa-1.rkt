...
(define (encrypt plaintext key)
  (cond
    ((eq? (string-length plaintext) 0) '())
    ((> (string-length plaintext) 0)(cons (mod-exp (char->integer (car (string->list plaintext))) (cadr key) (car key))
      (encrypt (list->string (cdr (string->list plaintext))) key)))
    ))
...
(define (decrypt ciphertext key)
  (list->string (map (lambda (x) (integer->char (mod-exp x (cadr key) (car key)))) ciphertext))
  )
