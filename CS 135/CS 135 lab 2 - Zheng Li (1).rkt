...
;; 2 PTS
(define (yoda words)
  (list (car (reverse words)) (car words) (car (cdr words))))
...
(define (rotate c)
  (list c
        (list (car (reverse c)) (- 0 (car c)) )
        (list (- 0 (car c)) (- 0 (car (reverse c))))
...
(define (combine-courses s1 s2)
  (append (car (cdr (cdr (cdr (cdr (cdr s1))))))
          (car (cdr (cdr (cdr (cdr (cdr s2))))))
        ))