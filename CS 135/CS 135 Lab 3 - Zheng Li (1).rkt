...
#|-------------------------------------------------------------------------------
 | Name:Zheng Li
 | Pledge:I pledge my honor that I have abided the Stevens Honors System
 |-------------------------------------------------------------------------------|#
...
;; 1 PTS
(define (piecewise x)
  (if (positive? x) (+ 1 (* 2 (expt x 2))) (+ x 1)))
...
(define (subfact n)
  (cond
    [(< n 1) "Error: Has to be a positive integer"]
    [(eq? n 1) 0]
    [(eq? n 2) 1]
    [(even? n) (+ 1 (* n (subfact (- n 1))))]
...
;;;;;;;;;;;;;;;;;Faster Method(called gen-fib);;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define x 0)
(define (add x1 x2 n1)
    (if (> n1 1)
      (and (set! x (+ x1 x2))(add x2 (+ x1 x2) (- n1 1)))
      x ))
(define (gen-fib a b n)
  (cond
    ...
    [(eq? n 1) b]
    [else (add a b n)]))
;;;;;;;;;;;;;;;;;Slower Method(called gen-fib2);;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (gen-fib2 a b n)
  (cond
    [(zero? n) a]
    [(eq? n 1) b]
    [else (+ (gen-fib2 a b (- n 1)) (gen-fib2 a b (- n 2)))]))
