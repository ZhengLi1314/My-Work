...
;; 3 PTS
(define (id n)
  (if (zero? n) '() (cons (list n n) (id (- n 1)))))
...
;; 3 PTS
(define (converse R)
  (map (lambda (relation) (reverse relation)) R))
...
(define (relate x R)
  (cond
    ((null? R) '())
    ((equal? (car (car R)) x) (append (cdr (car R)) (relate x (cdr R))))
    (else (relate x (cdr R)))
    ))
...
;; 3 PTS
(define (sec R)
  (if (null? R) '() (append (cdr (car R)) (sec (cdr R)))))
...
(define (injective? R)
  (cond
    ((null? R) #t)
    ((equal? 1 (length R)) #t)
    (else (and (not (contains (sec(cdr R)) (car (cdr (car R))))) (injective? (cdr R))))
    ))
