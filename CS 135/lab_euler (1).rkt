...
;; 4 PTS
(define (degree_helper G v)
  (cond
    ((null? G) '())
    (else (append (cond
                    ((and (eq? (caar G) v) (eq? (cadar G) v)) '(1 1))
                    ((or (eq? (caar G) v) (eq? (cadar G) v)) '(1))
                    (else '() ))
                  (degree_helper (cdr G) v)
                  ))
    ))

(define (degree G v)
  (length (degree_helper G v))
  )


#|(define (degree G v) (using map function (not finished))
  (map (lambda (x)
               (cond
                    ((and (eq? (car x) v) (eq? (cadr x) v)) '(1 1))
                    ((or (eq? (car x) v) (eq? (cadr x) v)) '(1))
                    (else '())))
               G))
|# 
...
(define (all-even? G)
  (cond
    ((null? G) #t)
    (else (all-even?_helper
           (map (lambda (x) (and (even? (degree G (car x))) (even? (degree G (cadr x))))) G)))
    ))

(define (all-even?_helper L)
  (cond
    ((null? L) #t)
    (else (and (car L) (all-even?_helper (cdr L)))))
  )
...
(define (adjacent G v)
  (cond
    ((null? G) '())
    (else (usort (append (cond
                    ((eq? (caar G) v) (list (cadar G)))
                    ((eq? (cadar G) v) (list (caar G)))
                    (else '() ))
                  (adjacent (cdr G) v)
                  )))
    ))
...
(define (connected? G)
  (cond
    ((null? G) #t)
    (else (all-even?_helper (map (lambda (x)
